...
/**
 * @author nguyentiendat file to run the task of language identity pick the
 *         language which most sentences in the document are written in.
 */
public class IdentityLang {

	public List<LangModel> loadModels() throws IOException,
			ClassNotFoundException {
		String dir = "./model/";
		if (!new File(dir).exists()) {
			System.out
					.println("Non exists model folder..! Please try build-model.jar first");
			return null;
		}
	...
	public CommandLine parseArguments02(String[] args) {
		...
		Option d = OptionBuilder.withArgName("testing document").hasArg()
